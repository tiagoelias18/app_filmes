...
class MovieDetailModel {
  ...
  factory MovieDetailModel.fromMap(Map<String, dynamic> map) {
    return MovieDetailModel(
      ...
      urlImages: List<dynamic>.from(map['images']['posters'] ?? const[]).map<String>((i) => 'https://image.tmdb.org/t/p/w200${i['file_path']}').toList(),
      releaseDate: DateTime.parse(map['release_date']),
      overview: map['overview'] ?? '',
      productCompanies: List<dynamic>.from(map['production_companies'] ?? const[]).map<String>((p) => p['name']).toList(),
